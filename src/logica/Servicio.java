...
public class Servicio {
    ...
    public Respuesta validar(Multa multa) {
        ...
        if (multa.getMulta().equalsIgnoreCase("Pico placa") && multa.getMonto() <= 130 | multa.getMonto() >= 350) {
            rpta.setCodigo(1);
            rpta.setMsj("El monto de Pico placa no esta en el rango 130 a 350");
            return rpta;
        }

        if (multa.getMulta().equalsIgnoreCase("Alta velocidad") && multa.getMonto() <= 400 | multa.getMonto() >= 570) {
            rpta.setCodigo(2);
            rpta.setMsj("El monto de Alta velocidad no esta en el rango 400 a 570");
            return rpta;
        }
        if (multa.getMulta().equalsIgnoreCase("Luz roja") && multa.getMonto() <= 130 | multa.getMonto() >= 250) {
            rpta.setCodigo(2);
            rpta.setMsj("El monto de Luz roja no esta en el rango 130 a 250");
            return rpta;
        }
        if (multa.getMulta().equalsIgnoreCase("Mal estacionado") && multa.getMonto() <= 100 | multa.getMonto() >= 190) {
            rpta.setCodigo(2);
            rpta.setMsj("El monto de Mal estacionado no esta en el rango 100 a 190");
            return rpta;
        }
        return rpta;
    }
    ...
    public Respuesta insertarMulta(Multa multa) {
        ...
        try {
            ...
            if (sumapunto+multa.getPunto() >= 100) {
                rpta.setCodigo(-1);
                rpta.setMsj("Multas superaron los 100 puntos");
                return rpta;
            }
